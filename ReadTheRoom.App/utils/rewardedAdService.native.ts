...
const getTestRecoveryFallback = (reason: string): RewardedAdResult =>
...
export async function showRewardedRecoveryAd(): Promise<RewardedAdResult> {
  if (isExpoGo()) {
    return getTestRecoveryFallback(
  ...
  try {
    ...
    return await new Promise<RewardedAdResult>((resolve) => {
      ...
      const timeoutId = setTimeout(() => {
        settle(getTestRecoveryFallback('Rewarded ad load timed out.'));
      }, AD_LOAD_TIMEOUT_MS);
      ...
      unsubscribers.push(
        ...
        rewardedAd.addAdEventListener(AdEventType.ERROR, (error) => {
          if (isRewardedAdTestMode()) {
            settle(getTestRecoveryFallback('Rewarded ad failed to load.'));
            return;
          }
          settle({ status: 'error', error });
        }),
